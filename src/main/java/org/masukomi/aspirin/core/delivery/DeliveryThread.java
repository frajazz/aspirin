...
public class DeliveryThread extends Thread
{
    ...
    public void run()
    {
        while( running )
        {
            synchronized( this )
            {
                if( dCtx == null )
                {
                     ...
                     */
                    {
                        if( dCtx != null )
                        {
                            AspirinInternal.getLogger().trace( "DeliveryThread ({}).run(): Release item after interruption. qi={}", new Object[]
                            {
                                getName(), dCtx
                            } );
            ...
            try
            {
                if( dCtx != null )
                {
                    AspirinInternal.getLogger().trace( "DeliveryThread ({}).run(): Call delivering... dCtx={}", new Object[]
                    {
                        getName(), dCtx
                    } );
    ...
    public void setContext( DeliveryContext dCtx ) throws MessagingException
    {
        ...
        synchronized( this )
        {
            ...
            this.dCtx = dCtx;
            AspirinInternal.getLogger().trace( "DeliveryThread ({}).setQuedItem(): Item was set. qi={}", new Object[]
            {
                getName(), dCtx
            } );
            notify();
    ...
    {
        AspirinInternal.getLogger().info( "DeliveryThread ({}).deliver(): Starting mail delivery. qi={}", new Object[]
        {
            getName(), dCtx
        } );
        ...
        for( String handlerName : handlerList )
        {
            ...
            catch( DeliveryException de )
            {
                ...
                AspirinInternal.getLogger().info( "DeliveryThread ({}).deliver(): Mail delivery failed: {}. qi={}", new Object[]
                {
                    getName(), qInfo.getResultInfo(), dCtx
                } );
                if( de.isPermanent() )
                {
        ...
        if( qInfo.hasState( DeliveryState.IN_PROGRESS ) )
        {
            ...
            AspirinInternal.getLogger().info( "DeliveryThread ({}).deliver(): Mail delivery success: {}. qi={}", new Object[]
            {
                getName(), qInfo.getResultInfo(), dCtx
            } );
