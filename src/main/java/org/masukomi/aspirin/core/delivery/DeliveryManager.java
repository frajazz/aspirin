...
public final class DeliveryManager extends Thread implements ConfigurationChangeListener
{
    ...
    public DeliveryManager()
    {
        ...
        GenericObjectPoolConfig gopConf = new GenericObjectPoolConfig();
        gopConf.setLifo( false );
        gopConf.setMaxTotal( AspirinInternal.getConfiguration().getDeliveryThreadsActiveMax() );
        gopConf.setMaxIdle( AspirinInternal.getConfiguration().getDeliveryThreadsIdleMax() );
        gopConf.setMaxWait( Duration.ofMillis( 5000L ) );
        gopConf.setTestOnReturn( true );
        gopConf.setBlockWhenExhausted( true );
            ...
            try
            {
                ...
                if( qi != null )
                {
                    ...
                    DeliveryContext dCtx = new DeliveryContext()
                            .setQueueInfo( qi )
                            .setMessage( message );
                    AspirinInternal.getLogger().trace( "DeliveryManager.run(): Pool state. A{}/I{}", new Object[]
                    {
                        deliveryThreadObjectPool.getNumActive(), deliveryThreadObjectPool.getNumIdle()
                    } );
                    try
                    {
                        ...
                        AspirinInternal.getLogger().trace( "DeliveryManager.run(): Borrow DeliveryThread object. dt={}: state '{}/{}'", new Object[]
                        {
                            dThread.getName(), dThread.getState().name(), dThread.isAlive()
                        } );
    ...
    public void release( QueueInfo qi )
    {
        ...
        AspirinInternal.getLogger().trace( "DeliveryManager.release(): Release item '{}' with state: '{}' after {} attempts.", new Object[]
        {
            qi.getMailid(), qi.getState().name(), qi.getAttemptCount()
        } );
    }
    ...
    public void configChanged( String parameterName )
    {
        synchronized( mailingLock )
        {
            ...
            else if( parameterName.equals( Configuration.PARAM_QUEUESTORE_CLASS ) )
            {
                queueStore = AspirinInternal.getConfiguration().getQueueStore();
            }
            if( parameterName.equals( ConfigurationMBean.PARAM_DELIVERY_THREADS_ACTIVE_MAX ) )
            {
                ( ( GenericObjectPool ) deliveryThreadObjectPool ).setMaxTotal( AspirinInternal.getConfiguration().getDeliveryThreadsActiveMax() );
            }
